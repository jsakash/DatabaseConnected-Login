...
func PostLogin(c *gin.Context) {
	...
	for _, i := range user {
		...
		if i.Username == Fusername && passwordIsValid && i.Status {
			status = true
			break
		}
	}
...
func PostSignup(c *gin.Context) {
	...
	db.Create(&Users{Username: FusernameN, Password: Fpassword, Status: true})
...
func BlockUser(c *gin.Context) {
	...
	name := c.Param("name")
	db := database.InitDB()
	db.Model(&user).Where("username=?", name).Update("status", updateData)
	c.Redirect(303, "/wadmin")
...
func UnblockUser(c *gin.Context) {
	...
	var unbStatus bool = true
	var user Users
	name := c.Param("name")
	db := database.InitDB()
	db.Model(&user).Where("username=?", name).Update("status", unbStatus)
	c.Redirect(303, "/wadmin")
